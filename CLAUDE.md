...
- **Streamlit Cloud's `WARN: More than one requirements file detected` is benign here, and was checked.** The repo offers it three candidates (`app/requirements.txt`, root `requirements.txt`, `pyproject.toml`); it resolves to `app/requirements.txt` because the entrypoint's directory wins, installing 42 packages with no xgboost/shap/scikit-learn/lightgbm/jupyter. Verified against the real build log on 2026-07-29. No root-level entrypoint shim is needed — don't add one, and don't re-investigate on seeing the warning.

The remaining work is optional and was agreed with the project owner on 2026-07-29, in this order: **Phase 12 (network analytics)** then **Phase 13 (streaming / online feature parity)** — both specified in `PLAN.md`'s Progress section. Neither is started. Phase 12 needs no new dependencies; Phase 13 is deliberately not a hosted service. Phase 10's screenshot item is closed — `reports/figures/11_app_alert_queue.png` and `11_app_alert_detail.png` are embedded in the README (screenshots must be captured manually; there is no browser tooling in this environment).
