...
- **Check:** open the live URL from a fresh/incognito session, click through every tab, confirm it matches local behavior. Add the link + a screenshot/GIF to the README. **All done** — incognito verified, link and two screenshots in the README (`reports/figures/11_app_alert_queue.png`, `11_app_alert_detail.png`). Before deploying, a fresh `git clone --depth 1` of the branch was run through `AppTest` to confirm the app boots from exactly what Streamlit Cloud receives (0 exceptions, 4 tabs, correct headline).
- **Deployment dependency resolution, verified from the build log (2026-07-29):** the repo offers Streamlit Cloud three candidate dependency files (`app/requirements.txt`, root `requirements.txt`, `pyproject.toml`) and it logs a `WARN: More than one requirements file detected`. It resolves to **`app/requirements.txt`** — the entrypoint's directory wins — installing 42 packages (the 5 declared plus Streamlit's own transitives) with none of xgboost/shap/scikit-learn/lightgbm/jupyter. The warning is benign; no root-level entrypoint shim is needed. Don't re-investigate this on seeing the WARN.
